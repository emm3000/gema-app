...
- Auto-merge (`gh pr merge --rebase --auto`) is intermittent on this repo. It stayed OPEN and CLEAN with green checks on PRs #181 and #185, even with `allow_auto_merge` on and no strict status checks. After arming it, the implementing session checks `gh pr view <N> --json state` once CI passes and pings the orchestrator if the PR is still open; the orchestrator then merges it by hand.
