...
- Also at cycle close, the orchestrator appends one row to `docs/agents/dispatch-log.md`: PR, issue, table row, model:effort, first-review verdict and, on FIX FIRST, whether the cause was a checklist item or a judgment error. The table in `.claude/skills/wave/SKILL.md` is tuned from that log, not from opinion. *Why: the table started as a bet (2026-09-16); without outcomes per row there is no way to tell which rows are too high or too low.*
