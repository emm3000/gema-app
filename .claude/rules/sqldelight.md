...
paths:
  ...
  - "app/src/test/java/com/emm/gema/migration/**"
  - "data/src/test/kotlin/com/emm/gema/migration/**"
---
...
- A change to any `.sq` that alters a `CREATE` statement ships three artifacts in the same commit: the `.sq` edit, `com/emm/gema/N.sqm` where `N` is the version before the bump, and `databases/(N+1).db`.
- Generate the snapshot with `./gradlew generateDebugGemaDbSchema` (task name updates to `:data:generateDebugGemaDbSchema` once the data module split lands). It writes the current version to `databases/`.
...
- Every `N.sqm` has a unit test under the migration test package (`app/src/test/java/com/emm/gema/migration/` today, `data/src/test/kotlin/com/emm/gema/migration/` once split) that builds schema `N` by hand, inserts legacy rows, migrates to `GemaDb.Schema.version` and asserts the rows survive.
