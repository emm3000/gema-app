...
class AttendanceUseCasesTest {
    ...
    fun `an untouched day shows every student as present and stores nothing`() = runTest {
        val entries: List<AttendanceEntry> = getAttendanceDay(SECTION_ID, today).first()
    ...
    fun `each of the four statuses is recorded for one student and one date`() = runTest {
        AttendanceStatus.entries.forEach { status: AttendanceStatus ->
            recordAttendance(SECTION_ID, luz.id, today, status)

            val entry: AttendanceEntry = getAttendanceDay(SECTION_ID, today).first().single { it.student.id == luz.id }
    ...
    fun `a past date is recorded and read back on its own date`() = runTest {
        recordAttendance(SECTION_ID, luz.id, yesterday, AttendanceStatus.LATE)

        assertThat(getAttendanceDay(SECTION_ID, yesterday).first().single { it.student.id == luz.id }.status)
            .isEqualTo(AttendanceStatus.LATE)
        assertThat(getAttendanceDay(SECTION_ID, today).first().single { it.student.id == luz.id }.isRecorded)
            .isFalse()
    }
    ...
    fun `a future date is refused`() = runTest {
        val outcome: Result<Unit> = runCatching {
            recordAttendance(SECTION_ID, luz.id, tomorrow, AttendanceStatus.PRESENT)
        }
    ...
    fun `a withdrawn student is listed before the withdrawal date and gone from it on`() = runTest {
        ...
        assertThat(getAttendanceDay(SECTION_ID, yesterday).first().map { it.student.id })
            .containsExactly(luz.id, jose.id)
        assertThat(getAttendanceDay(SECTION_ID, today).first().map { it.student.id }).containsExactly(luz.id)
    }

    @Test
    fun `a withdrawn student keeps the records taken before the withdrawal`() = runTest {
        recordAttendance(SECTION_ID, jose.id, yesterday, AttendanceStatus.JUSTIFIED)

        studentRepository.save(jose.copy(withdrawalDate = today))

        assertThat(getAttendanceDay(SECTION_ID, yesterday).first().single { it.student.id == jose.id }.status)
    ...
    fun `recorded days are counted once per date`() = runTest {
        ...
        recordAttendance(SECTION_ID, luz.id, today, AttendanceStatus.ABSENT)
        recordAttendance(SECTION_ID, jose.id, today, AttendanceStatus.LATE)
        recordAttendance(SECTION_ID, luz.id, yesterday, AttendanceStatus.ABSENT)

        assertThat(countAttendanceDays(SECTION_ID)).isEqualTo(2)
    }
}
