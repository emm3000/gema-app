...
class SectionUseCasesTest {
    ...
    fun `deleting a section drops the siagie template imported for it`() = runTest {
        val section: Section = createSection(SchoolYearId("2026"), Grade.THIRD, "A").let {
            getSections(SchoolYearId("2026")).first().single()
        }
    ...
    fun `a created section is listed under its school year`() = runTest {
        createSection(schoolYearId = SchoolYearId("2026"), grade = Grade.THIRD, name = "A")

        val sections: List<Section> = getSections(SchoolYearId("2026")).first()

        assertThat(sections.single().grade).isEqualTo(Grade.THIRD)
        assertThat(sections.single().name).isEqualTo("A")
        assertThat(getSections(SchoolYearId("2025")).first()).isEmpty()
    }

    @Test
    fun `sections are listed by grade and then by name`() = runTest {
        createSection(SchoolYearId("2026"), Grade.SECOND, "B")
        createSection(SchoolYearId("2026"), Grade.FIRST, "B")
        createSection(SchoolYearId("2026"), Grade.SECOND, "A")
    ...
    fun `a renamed section keeps its id`() = runTest {
        val created: Section = createSection(SchoolYearId("2026"), Grade.FIRST, "A")

        updateSection(sectionId = created.id, grade = Grade.FOURTH, name = "Unica")

        val stored: Section = getSections(SchoolYearId("2026")).first().single()
    ...
    fun `updating a section that does not exist is rejected`() = runTest {
        updateSection(sectionId = SectionId("missing"), grade = Grade.FIRST, name = "A")
    }
    ...
    fun `a deleted section disappears with its area visibility`() = runTest {
        ...
        assertThat(getSections(SchoolYearId("2026")).first()).isEmpty()
    ...
    fun `hiding an area of one section leaves the other sections untouched`() = runTest {
        val first: Section = createSection(SchoolYearId("2026"), Grade.FIRST, "A")
        val second: Section = createSection(SchoolYearId("2026"), Grade.SECOND, "B")
